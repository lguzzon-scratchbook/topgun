import {
    ConsumableStream,
    type ConsumableStreamConsumer,
    type DemuxedConsumableStream,
    type StreamDemux
...
import type { TGExchange } from './exchange'
import type { TGStreamState } from './types'
...
export class TGStream<T> extends ConsumableStream<T> 
{
   ...
   */
    ...
    ) 
    {
        ...
        {
            const collectionOptions = this.attributes[
                'topGunCollection'
            ] as TGCollectionOptions

            const eventPublish = event =>
      ...
      ;(async () => 
            {
                for await (const { key, value } of this
                    ._dataStream as DemuxedConsumableStream<TGData<TGNode>>) 
                {
                    // Detect changes
                    const emptyChange =
            !value && (this.nodes.length === 0 || !this.existingNodesMap[key])
                    ...
                    const nodeNotChanged =
            exists && valueIsNode && !diffCRDT(updatedGraph, existingGraph)
                    ...
                    if (valueIsNode) 
                    {
                        ...
                        {
                            node[collectionOptions.idField] = getNodeSoul(node)
                                .split('/')
                                .pop()
                        }
    ...
    #getNodeIndex(soul: string) 
    {
        return this.nodes.findIndex(
            node => getNodeSoul(node) === getNodeSoul(this.existingNodesMap[soul])
        )
    }
}
