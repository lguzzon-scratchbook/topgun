import { isObject, isString, isUndefined } from '@topgunbuild/typed'
import type { TGClient, TGLink } from '../client'
import type {
    TGEncryptData,
    TGGraphData,
    TGNode,
...
import type { PairBase } from './pair'
...
): Promise<TGGraphData> 
{
    ...
    for (const soul in graph) 
    {
        ...
        cert.m &&
        cert.s &&
        (await verifyCertificate(client, cert, pair.pub, soulPub, fullPath))
            ) 
            {
                modifiedGraph[soul] = await signNode(node, pair)
            }
